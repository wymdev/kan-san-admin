    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('secondary-transactions.store') }}" method="POST" id="transactionForm">
                ...
                {{-- Sale Type Selection --}}
                <div class="mb-6">
                    <h6 class="text-base font-semibold mb-3 flex items-center gap-2"><i class="size-4" data-lucide="tag"></i> Sale Type</h6>
                    <input type="hidden" name="sale_type" id="saleTypeInput" value="own">
                    
                    <div class="grid md:grid-cols-2 gap-4">
                        <div class="sale-type-card selected p-4 rounded-xl bg-white border" data-type="own" onclick="selectSaleType('own')">
                        ...
                        <div class="sale-type-card p-4 rounded-xl bg-white border" data-type="other" onclick="selectSaleType('other')">
                            <div class="flex items-center gap-3">
                ...
                {{-- Ticket Selection --}}
                <div class="mb-6">
                    <h6 class="text-base font-semibold mb-3 flex items-center gap-2"><i class="size-4" data-lucide="ticket"></i> Select Ticket</h6>
                    ...
                    @if(isset($selectedTicket))
                        ...
                        <div class="p-4 bg-primary/5 rounded-lg border border-primary/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <span class="font-mono text-2xl font-bold text-primary tracking-widest">{{ $selectedTicket->ticket_number }}</span>
                                    @if($selectedTicket->withdraw_date)
                                        <p class="text-sm text-default-500 mt-1">Draw: {{ $selectedTicket->withdraw_date->format('M d, Y') }}</p>
                                    @endif
                                </div>
                                <a href="{{ route('secondary-transactions.create') }}" class="text-sm text-primary hover:underline">Change</a>
                            </div>
                        </div>
                    @else
                        <select name="secondary_ticket_id" id="ticketSelect" class="form-select @error('secondary_ticket_id') border-danger @enderror" required>
                ...
                <div id="customerSection" class="customer-section mb-6">
                    ...
                    <div class="p-3 bg-default-100 rounded-lg mb-4">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                <label class="form-label">Phone Number <span class="text-xs text-default-400">(optional)</span></label>
                                <input type="text" name="customer_phone" id="customerPhone" value="{{ old('customer_phone') }}" 
                                       class="form-input" placeholder="e.g., 0923471220">
                            </div>
                            <div>
                        ...
                        <div class="mt-3">
                            <label class="flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm text-default-600">Create customer account if not exists (default password: password123)</span>
                            </label>
                        </div>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="amount_thb" value="{{ old('amount_thb') }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="amount_mmk" value="{{ old('amount_mmk') }}" 
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" rows="2" class="form-input" placeholder="Any additional notes...">{{ old('notes') }}</textarea>
                </div>
