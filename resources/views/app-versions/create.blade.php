@extends('layouts.vertical', ['title' => 'Create App Version'])

@section('content')
    @include('layouts.partials/page-title', ['subtitle' => 'Mobile App', 'title' => 'Create New Version'])
    ...
    <div class="grid grid-cols-1 gap-5">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('app-versions.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" id="is_active" class="form-checkbox" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
                            <label for="is_active" class="form-label mb-0">Active</label>
                        </div>

                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_latest" id="is_latest" class="form-checkbox" value="1" {{ old('is_latest') ? 'checked' : '' }}>
                            <label for="is_latest" class="form-label mb-0">Mark as Latest</label>
                        </div>

                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="force_update" id="force_update" class="form-checkbox" value="1" {{ old('force_update') ? 'checked' : '' }}>
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="btn bg-primary text-white">
                            <i class="size-4 me-1" data-lucide="save"></i>Create Version
                        </button>
