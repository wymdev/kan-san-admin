                ...
                <form action="{{ route('app-versions.update', $version) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <x-ui.input type="text" name="version" id="version" class="form-input " value="{{ old('version', $version->version) }}" placeholder="e.g., 1.0.0" required />
                            @error('version')
                        ...
                        <div>
                            ...
                            <x-ui.input type="number" name="version_code" id="version_code" class="form-input " value="{{ old('version_code', $version->version_code) }}" placeholder="e.g., 1" required />
                            @error('version_code')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <x-ui.input type="datetime-local" name="release_date" id="release_date" class="form-input " value="{{ old('release_date', $version->release_date?->format('Y-m-d\TH:i')) }}" />
                            @error('release_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <x-ui.input type="text" name="minimum_version" id="minimum_version" class="form-input " value="{{ old('minimum_version', $version->minimum_version) }}" placeholder="e.g., 0.9.0" />
                        ...
                        <div>
                            ...
                            <x-ui.input type="number" name="minimum_version_code" id="minimum_version_code" class="form-input " value="{{ old('minimum_version_code', $version->minimum_version_code) }}" placeholder="e.g., 9" />
                    ...
                    <div class="mb-4">
                        ...
                        <x-ui.input type="url" name="download_url" id="download_url" class="form-input " value="{{ old('download_url', $version->download_url) }}" placeholder="https://play.google.com/store/apps/..." />
                        @error('download_url')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <x-ui.textarea name="release_notes" id="release_notes" rows="4" class="form-input " placeholder="What's new in this version...">{{ old('release_notes', $version->release_notes) }}</x-ui.textarea>
                        @error('release_notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            @if(old('features', $version->features))
                                @foreach(old('features', $version->features ?? []) as $feature)
                                    <div class="flex gap-2 mb-2">
                                        <x-ui.input type="text" name="features[]" class="form-input" value="{{ $feature }}" placeholder="Add a new feature..." />
                        ...
                        <div id="bugfixes-container">
                            @if(old('bug_fixes', $version->bug_fixes))
                                @foreach(old('bug_fixes', $version->bug_fixes ?? []) as $bugFix)
                                    <div class="flex gap-2 mb-2">
                                        <x-ui.input type="text" name="bug_fixes[]" class="form-input" value="{{ $bugFix }}" placeholder="Add a bug fix..." />
                                        <button type="button" class="btn btn-sm bg-red-500 text-white" onclick="this.parentElement.remove()">
                                            <i class="size-4" data-lucide="minus"></i>
                                        </button>
                                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <x-ui.input type="number" name="display_order" id="display_order" class="form-input " value="{{ old('display_order', $version->display_order) }}" />
                        @error('display_order')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
